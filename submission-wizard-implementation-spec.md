...
There is no execution-environment choice in either wizard. Legacy `worker` request values canonicalize to Vercel before admission, and a pre-existing stored job without protected budget authority fails closed with instructions to submit it again. Keep an internal sandbox-provider seam so infrastructure can later move to a cheaper equivalent isolation provider after separate security, accounting and compatibility verification; this is deployment policy, not a user-facing optimizer setting.
...
- Completed-run interactions: protected serving and ad hoc evaluation need fresh paid-operation authority after the original run reaches a terminal state. Decide whether each request gets an explicit one-request budget or draws from a separately funded reusable playground budget, and decide whose budget funds an invocation made through an editor share link. Until this is settled, protected interactive execution remains blocked while secret-free metadata stays readable.
...
- Protected completed-job routes return persisted, credential-scrubbed metadata without materializing authored code or pickle artifacts in the API process. Interactive inference, streaming, ReAct, evaluation, and shared serving remain blocked until their fresh budget authority is implemented.
- Verification has used deterministic provider transports, local loopback transport checks, disposable databases, and actual Linux isolation with network disabled. Read-only OpenRouter authentication and the configured Vercel Sandbox project query succeeded. A private `skynet-runtime` repository now exists in the `skynet-blackbox` Vercel project and authenticated registry login succeeds; it is still empty, so immutable image publication and a live sandbox run remain unverified. No real paid model call, paid Vercel smoke test, deployment, or production verification has yet been recorded for this implementation branch.

Remaining integration work must be closed before describing this specification as fully implemented: dependency preparation, safe funding for completed-run interactions, immutable deployed sandbox images, and controlled live provider/Vercel evidence. PR approval and deployment approval remain separate from local implementation.
