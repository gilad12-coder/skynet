...
Vercel does not execute Docker `CMD` or `ENTRYPOINT`; Skynet starts the runner explicitly. `WORKDIR` is honored. The parent stages its own backend package into every session and starts the guest from it with `PYTHONSAFEPATH` set, so the working directory `/app` cannot shadow the shipped modules with the image's own copy and the image pins Python and the optimizer dependencies while the backend modules follow the worker. The guest verifies Python patch version, installed optimizer dependencies, and backend source hash against its parent before executing authored code, and names each mismatched part when it refuses. Rebuild and re-pin the image when the Python version or a dependency lock changes; a backend source change needs no rebuild. Image publication and dependency installation alone do not establish that a live setup check succeeded. [Vercel image documentation](https://vercel.com/docs/sandbox/concepts/images)
