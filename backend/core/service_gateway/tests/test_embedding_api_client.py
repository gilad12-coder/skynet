...
def test_embedding_api_client_normalizes_openai_response() -> None:
    ...
    with (
        ...
        patch.object(embeddings.settings, "embeddings_api_key", SecretStr("embed-secret")),
...
def test_embedding_api_client_requires_configured_base_url() -> None:
    """An unconfigured API degrades to unavailable; ``encode`` returns ``None``."""
    with (
        patch.object(embeddings.settings, "embeddings_base_url", ""),
        patch.object(embeddings.settings, "embeddings_model", "embed-model"),
    ):
        client = embeddings._EmbeddingApiClient()
...
def test_embedding_api_client_rejects_short_vectors() -> None:
    ...
    with (
        patch.object(embeddings.settings, "embeddings_base_url", "https://llm.internal/v1"),
        patch.object(embeddings.settings, "embeddings_model", "embed-model"),
        patch.object(embeddings.settings, "embeddings_dim", 3),
        patch.object(embeddings.requests, "post") as post,
    ):
        response = Mock()
