...
ENGLISH_MESSAGES: dict[str, str] = {
    ...
    "accounts.signups_closed": "New sign-ups are paused right now. Please check back soon.",
    ...
    "billing.openrouter_oauth_failed": "OpenRouter sign-in didn't complete. Try again.",
    "billing.openrouter_oauth_state_invalid": "The OpenRouter sign-in link is invalid or expired. Start again.",
    "billing.openrouter_unreachable": "OpenRouter could not be reached. Try again in a moment.",
    ...
    "billing.already_subscribed": "You're already on Skynet Pro. Manage your plan from Billing.",
    "billing.not_configured": "Billing is not configured on this deployment.",
    "billing.plan_unavailable": "Skynet Pro isn't available on this deployment yet.",
    ...
    "connectors.vault_not_configured": "The credential vault is not configured on this deployment, so connectors can't be linked.",
    "connectors.reconnect_required": "Your {provider} link can no longer be read. Disconnect and connect again.",
    "connectors.hf_oauth_not_configured": "Sign-in with Hugging Face isn't set up on this deployment. Paste an access token instead.",
    "connectors.hf_not_connected": "Connect your Hugging Face account first.",
    "connectors.hf_invalid_token": "Hugging Face rejected that token. Check it and try again.",
    "connectors.hf_unreachable": "Hugging Face could not be reached. Try again in a moment.",
    "connectors.hf_dataset_not_found": "Dataset {repo_id} wasn't found on Hugging Face, or your account can't access it.",
    "connectors.hf_dataset_unsupported": "Hugging Face can't serve {repo_id} yet (no viewer data or parquet conversion).",
    "connectors.hf_expired": "Your Hugging Face session expired. Connect again from Settings.",
    "connectors.hf_oauth_failed": "Hugging Face sign-in didn't complete. Try again.",
    "connectors.hf_oauth_state_invalid": "The Hugging Face sign-in link is invalid or expired. Start again.",
    "connectors.hf_import_too_large": "That split is too large to import (the compressed-file limit is {max_mb} MB).",
    ...
    "dataset.split.rationale.best_of_n.capped": "This is a large dataset ({total} examples). Best-of-N scores every proposal on the val set, so it is capped at {val_count} examples to keep each proposal affordable; {test_count} are held out as test, and the remaining {train_count} stay in train, which Best-of-N doesn't use.",
    "dataset.split.rationale.best_of_n.holdout": "Best-of-N never trains — it scores every proposal on the val set — so {val_count} examples go to val and {test_count} are held out as test to check the winner.",
    "dataset.split.rationale.best_of_n.pooled": "This dataset is very small ({total} examples). Best-of-N never trains — it scores every proposal on a single set — so all examples go into that set, which also serves as the holdout.",
    ...
    "dataset.split.rationale.meta_harness.holdout": "Meta-Harness runs the agent on train and val cases alike, so they are pooled: {train_count} examples drive the optimization and {test_count} are held out as test to check the winner.",
    "dataset.split.rationale.meta_harness.pooled": "This dataset is very small ({total} examples). Meta-Harness runs the agent on train and val cases alike, so all examples go into one pool, which also serves as the holdout.",
