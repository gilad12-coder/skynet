    ...
    def login(
        ...
        Raises:
            DomainError: 403 on a bad internal secret, or when the account's
                ...
                401 when the email is unknown, the password does not match, or
                the account's second factor is missing
                (``accounts.two_factor_required``, carrying the usable methods
                in ``params``) or wrong (``accounts.invalid_second_factor``).
        """
        _require_internal_auth(x_internal_auth)
        email = _normalise_email(body.email)
        with Session(job_store.engine) as session:
