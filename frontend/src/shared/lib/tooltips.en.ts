...
export const TOOLTIPS_EN: Record<TooltipKey, string> = {
  "score.baseline":
    "Baseline score before optimization: how the program did without an improved prompt or selected examples",
  "score.optimized":
    "Optimized score after optimization: how the program did with the chosen prompt and examples",
  "score.improvement":
  ...
  "lm_activity.stage.training":
    "Calls made during the optimization itself — while the optimizer built prompt candidates",
  "lm_activity.stage.evaluation":
    "Calls made while measuring the optimized score — after the optimization finished",
  ...
  "model.reflection":
    "The model that checks mistakes and suggests how to improve the instructions during optimization",

  "data.split_explanation":
    "The dataset is split into three parts: train for learning, val for choosing the prompt, and test for the final measurement",
  "data.shuffle_explanation":
    "Shuffles the order of rows before the split, so the file order does not accidentally affect the results",
  ...
  "data.split.test":
    "Examples reserved for the final measurement, after the prompt has been chosen",
  "data.seed":
    "A fixed starting number that keeps the same split and the same shuffle on every repeated run",

  "prompt.optimized":
    "The prompt the optimizer built: improved instructions and examples selected from the dataset",
  "prompt.demonstrations":
    "Input-output examples (few-shot demonstrations) shown to the model to show it the desired format and answer",
  ...
  "module.predict":
    "Predict — the basic module: maps the input to the output in a single call to the model, with no intermediate steps",
  "module.cot":
  ...
  "optimizer.choice":
    "The method that tries to improve the prompt and find a version with a higher score",

  "react.tool_source":
    "Where the tool list is loaded from: a live MCP server, or a snapshot of tools from the dataset",
  "react.mcp_url": "The address of the MCP server from which the agent's tools are loaded",
  "react.auth":
    "Authentication header (Authorization header) for the MCP server. Not stored on the server and not exposed to the chat agent",
  "react.optimized_tools":
    "The tools the agent (ReAct) runs in a loop, with the descriptions and arguments the optimizer refined during optimization",
  ...
  "serve.api_url_run":
    "The API URL you send POST requests to with the input fields in order to get a prediction from the improved program",
  "serve.api_url_react":
  ...
  "model_config.connection_section":
    "Run the model on your own server: an OpenAI-compatible endpoint (Ollama, vLLM, LM Studio or an enterprise gateway) and an access key. Leave closed to use the built-in providers",
  "model_config.model":
    "The model that will run the optimization. Choose from the model catalog, or a custom model discovered from the Base URL",
  "model_config.base_url":
    "The address of your own OpenAI-compatible server — Ollama, vLLM, LM Studio or an enterprise gateway. Leave empty to use the provider's default server",
  "model_config.api_key":
    "Access key for the model server. Optional — if empty, it is taken from an environment variable. Not stored on the server and removed from the form after submission",
  "model_config.temperature":
    "How creative the model is — a low value gives consistent answers, a high one more varied",
  "model_config.top_p":
    "top_p (nucleus sampling): limits the range of words the model considers — a low value narrows it, a high one allows more variety",
  "model_config.max_tokens": "The maximum prediction length — a token is roughly one word",

  "code.signature_metric":
    "The source code of the Signature and metric defined for this optimization",
  "code.signature":
    "Defines the input and output fields of the task — what the model receives and what it needs to return",
  "code.metric":
    "A function that measures the quality of the prediction — returns a numeric score for each example",
  "code.predictions_table":
    "Results of running the program on the test examples — a score for each example and an overall summary",

  "tagger.upload_file": "Upload a CSV, JSON or Excel file. Each row becomes an item to tag",
  "tagger.text_column":
    "Choose the column that contains the text to tag. The other columns are kept in the export",
  "tagger.mode":
    "Choose the tagging type that fits the task: yes/no, selection from a list, or free text",
  ...
  "tagger.multiclass_categories":
    "Define the categories available for selection while tagging — at least two",
  "tagger.freetext_instruction":
    "The instruction shown above the text field. Briefly explain what needs to be written",

  "compare.winner_improvement":
    "The score improvement percentage of the winning run — the difference between the optimized score and the baseline score",
  ...
  "analytics.runtime_vs_gain":
    "Analysis of run times and efficiency — how much improvement is gained relative to time",
  "analytics.runtime_minutes":
    "The duration of the run in minutes for every completed optimization",
  "analytics.improvement_per_minute":
    "Score improvement percentage per minute of run — a high value means a more efficient optimization",
  "analytics.dataset_size_vs_improvement":
    "Whether more data leads to better score improvement — each point is one optimization",
  "analytics.submissions_per_day": "The number of optimizations submitted per day",
  "analytics.optimizer_avg_improvement":
    "Average score improvement percentage each optimizer achieved across all runs",
  "analytics.top_improvements":
    "The runs that achieved the largest improvement in score, from best to least",
  "analytics.optimizer_comparison_table":
    "Detailed comparison between the optimizers: average improvement, number of runs, and run time",
  "analytics.model_performance_table":
    "Performance of the different models: usage frequency and average improvement",
};
