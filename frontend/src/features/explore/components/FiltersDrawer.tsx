...
/** A field in the drawer; the facet dimensions plus the date range. */
 ...
 * would leave alongside the other active filters. Typing searches the whole
 * dimension server-side, and the selection stays pinned at the top of its
 * list however far it ranks. Every pick applies immediately; the primary
 * button reports the live result count so the effect is visible before the
 * panel closes.
 */
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      ...
      >
        <div className="flex h-full min-h-0 flex-col">
          <SheetHeader className="flex-row items-center justify-between gap-3 px-6 pt-5 pb-3">
              ...
              >
                <div className="flex flex-col gap-3 pb-4">
                  <label className="flex flex-col gap-1.5">
                    ...
                    <SkynetDatePicker
                      value={dateFrom}
                      onChange={(next) => onChangeDateRange(next, dateTo)}
                      max={dateTo}
                  ...
                  <label className="flex flex-col gap-1.5">
                    ...
                    <SkynetDatePicker
                      value={dateTo}
                      onChange={(next) => onChangeDateRange(dateFrom, next)}
                      min={dateFrom}
                      ariaLabel={msg("explore.filters.date.to")}
                    />
                  </label>
                  {dateCount > 0 && <ClearSelection onClick={() => onChangeDateRange(null, null)} />}
                </div>
              </FieldRow>
            </div>
          </div>
